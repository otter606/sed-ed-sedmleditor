...
The features listed above exist in all versions of SED-ED. If you run SED-ED as an SBSI plugin, then you will also be able to execute SED-ML files and run simulations as well. We'll cover this as we go along.
...
# Unzip and you should get a folder with the following contents: !images/installationDir.png!. 
# To start the application, double-click the 'SED-ED' launch icon.
...
# The application will now open to produce a rather empty-looking window. !{width:70%}images/initView.png!
...
Give the file a name, and a project to which it will belong. *Make sure the file name ends with the suffix .xml* . 
...
Now, select the Plot2D shape, right-click and choose 'Add Curve' from the menu. We'll just be adding one curve in this example. The curve will appear in red on the diagram. By now you know this means there is still work to be done - in this case, we need to join the curve shape to the two data-generators that will produce the axes of the curve. Connect the curve to the Time(X) and P0(Y) data generators, ending up with something like this:
...
If you're running SED-ED from within SBSIVisual, and are networked, you can now select the SED-ML file in the System View, right click and run the simulation via the 'Execute SED-ML' menu. You should see the  your labours rewarded with a graph something like this:
...
3. Double click the  model copy to open the configuration dialog, and set the 'source' to be the value of the ID of the original model. If you do this correctly the 'Show Preview' button should be active. Click *OK*.
...
To generate a new output showing the simulation output of the new model, make a copy of the  existing  tasks, dataGenerators, and outputs, and connect the new model to the new copy of  the task. You may need to edit the new Variable Ids and Math expressions - every ID must be globally unique in SED-ML.
