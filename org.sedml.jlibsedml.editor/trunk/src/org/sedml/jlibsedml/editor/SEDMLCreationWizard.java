 ...
 */
public class SEDMLCreationWizard extends Wizard implements INewWizard {
	...
	private class CreationPage extends WizardNewFileCreationPage {
		 ...
		 * @see SEDMLCreationWizard#init(IWorkbench, IStructuredSelection)
		 */
		CreationPage(IWorkbench workbench, IStructuredSelection selection) {
			super("SEDMLCreationPage1", selection);
			this.workbench = workbench;
			setTitle("Create a new SEDML " + DEFAULT_EXTENSION + " file");
			setDescription("Create a new  SEDML " + DEFAULT_EXTENSION + " file");
		}
		 ...
		 * 
		 * @see SEDMLCreationWizard#performFinish()
		 */
		boolean finish() {
		...
		protected InputStream getInitialContents() {
			ByteArrayInputStream bais = null;
				...
				String doc = Libsedml.createDocument().writeDocumentToString();
				bais = new ByteArrayInputStream(doc.getBytes());
