...
public class AddNoteDialog extends BaseConfigDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		if(sedmlEl.getNotes()!=null){
			...
			text.setText(handler.getElementsAsString(Arrays.asList(new Element[]{el})));
		}
	...
	protected void okPressed() {
		...
		if(hasChanged(textStr)) {
			execute(new ICommand() {
				...
				public void redo() {
					sedmlEl.setNotes(new Notes(handler.getXMLElements(textStr).get(0)));				
				}
	...
	private boolean hasChanged(String textStr) {
		...
		return !textStr.equals(handler.getElementAsString(oldNotes.getNotesElement()));
	}
